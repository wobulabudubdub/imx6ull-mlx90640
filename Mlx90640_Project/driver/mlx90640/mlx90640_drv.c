...
static int mlx90640_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0) {
        dev_err(&client->dev, "Failed to read CTRL1: %d\n", ret);
        goto err_free;
    }
    ...
    ret = mlx90640_write_reg16(client, MLX90640_REG_CTRL1, ctrl1);
    if (ret < 0) {
        dev_err(&client->dev, "Failed to write CTRL1: %d\n", ret);
        goto err_free;
    }

    ret = alloc_chrdev_region(&mlx_dev->devid, 0, MLX90640_CNT, MLX90640_NAME);
    if (ret < 0) {
        dev_err(&client->dev, "alloc_chrdev_region failed: %d\n", ret);
        goto err_free;
    }
    dev_info(&client->dev, "Major=%d Minor=%d\n",
             MAJOR(mlx_dev->devid), MINOR(mlx_dev->devid));

    cdev_init(&mlx_dev->cdev, &mlx90640_fops);
    ret = cdev_add(&mlx_dev->cdev, mlx_dev->devid, MLX90640_CNT);
    if (ret < 0) {
