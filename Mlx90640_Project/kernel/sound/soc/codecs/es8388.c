 ...
 * Author: Jianquan Wang <wangjianquan@alientek.com>
 *  
...
static int es8388_set_gpio(struct es8388_priv *es8388, int gpio, bool level)
{
    unsigned int pwr_reg;

    if ((gpio & ES8388_CODEC_SET_SPK) && es8388->hp_det_gpio) {

        pwr_reg = snd_soc_read(es8388_codec, ES8388_DACPOWER);

        if(level) 
        {
            snd_soc_write(es8388_codec, ES8388_DACPOWER, 0x0C);
        }
        else
        {
            snd_soc_write(es8388_codec, ES8388_DACPOWER, 0x30);
        }
    }
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	dev_dbg(es8388_codec->dev,"%s \n", __func__);

	if (es8388->muted == 0) {
		if (gpiod_get_value(es8388->hp_det_gpio))
		{
			es8388_set_gpio(es8388, ES8388_CODEC_SET_SPK, 0);
		}
		else 
		{
			es8388_set_gpio(es8388, ES8388_CODEC_SET_SPK, 1); 
		}
	}
                ...
                const struct i2c_device_id *id)
{
    ...
    ret = i2c_smbus_read_byte_data(i2c, 0x00);
    if (ret < 0) {
        dev_err(&i2c->dev, "Failed to communicate with ES8388 chip\n");
        return -ENODEV;
    }
	...
	if (hp_irq < 0) {
        dev_err(&i2c->dev, "Failed to get IRQ number for hp-det-gpio\n");
        return hp_irq;
    }

	ret = devm_request_threaded_irq(&i2c->dev, hp_irq, NULL,
									hp_det_irq_handler, IRQ_TYPE_EDGE_BOTH  | IRQF_ONESHOT ,"hp_det_gpio", es8388);
	if (ret < 0) {
    ...
    INIT_DELAYED_WORK(&es8388->pcm_pop_work, pcm_pop_work_events);
...
MODULE_AUTHOR("Jianquan Wang <wangjianquan@alientek.com>");
MODULE_LICENSE("GPL");
