...
static s16 mlx90640_calc_ta(struct mlx90640_dev *dev, u16 *raw)
{
    struct mlx90640_calibration *c = &dev->calib;
    s32 ptat;
    s32 ta;
    ...
    /* VDD 补偿 (仅用于完整算法时可扩展) */
    raw_vdd = (c->vdd_25 + raw_vdd) * c->k_vdd / 32768;
    raw_vdd = raw_vdd / 64; /* 10mV 单位 */

    ptat = (s32)raw[816];
    if (ptat > 32767)
        ptat -= 65536;

    ta = (ptat / (ptat * c->alpha_ptat + 512)) - 256;
    ta = ta * 100; /* 转成 0.01°C */

    return (s16)ta;
}

static s16 mlx90640_calc_to(struct mlx90640_dev *dev, int pixel,
                             s16 ta, u16 *raw)
{
    struct mlx90640_calibration *c = &dev->calib;
    s32 vir_comp;
    ...
    raw_pixel = (s16)raw[pixel];
    if (raw_pixel > 32767)
        raw_pixel -= 65536;

    vir_comp = raw_pixel - c->offset[pixel];
    vir_comp = vir_comp * (1 + c->kta[pixel] * ta / 100);
    vir_comp = vir_comp * (1 + c->kv[pixel] * ta / 100);

    alpha_corr = c->alpha[pixel];
    if (alpha_corr == 0)
        alpha_corr = 1;

    temp = vir_comp * 10000 / alpha_corr;
    temp = temp + (s32)ta * 100;
...
static int mlx90640_process_frame(struct mlx90640_dev *dev)
{
    ...
    ta = mlx90640_calc_ta(dev, dev->raw_frame);
    dev->ambient_temp = ta;

    for (i = 0; i < MLX90640_PIXELS; i++) {
        dev->frame_data[i] = mlx90640_calc_to(dev, i, ta, dev->raw_frame);
    }
